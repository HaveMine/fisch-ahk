...
thegui2.BackColor := "1E1E2E" ; Samakan warna background
thegui2.SetFont("s14 c89b4fa w700", "Segoe UI")
letext := thegui2.add("text", "y10 w160 Center", "Settings")

thegui2.SetFont("s10 c585b70", "Segoe UI")
legroupboxthatcontrolitall := thegui2.add("groupbox", "x10 y45 w140 h180")

thegui2.SetFont("s10 ccdd6f4 w600", "Segoe UI") ; Text putih
dothisnumberalign := thegui2.add("text","x20 y60 w120 Center","Number Color")

; Edit Box
thegui2.SetFont("s10 cBlack", "Segoe UI")
thepixelcolor := thegui2.add("edit", "x30 y85 w100 +Center", thecolorforthenumberthing)

; Button Get Color
getthepixelcolor := thegui2.add("button", "x30 y115 w100 h25", "Get Color")
getthepixelcolor.onevent("click", (*) => GetTheColorForSomething())

; Text Tolerance
thegui2.SetFont("s10 ccdd6f4 w600", "Segoe UI")
dangitsthecolortolerance := thegui2.add("text","x20 y150 w120 Center","Tolerance")

; Edit Tolerance
thegui2.SetFont("s10 cBlack", "Segoe UI")
thetolerancething := thegui2.add("edit", "x50 y175 w60 +Center", thetolerancesettingsyeah)

; Save Button
savethegui2settings := thegui2.add("button", "x20 y240 w120 h35", "Save Settings")
...
global
	...
	if (XHalf or YHalf) {
		...
		msgbox "Press `"F7`" first to set a box to the fish", "ERROR", "0x40030"
		try winactivate("Roblox")
	}
}
...
global
	if (IsStoppingNoYes) {
		Highlight()
		Highlight2()
		msgbox "Auto Appraise have been stopped.", "Stopped", "0x40040"
...
global
	maps := map()
	reachedmax := 0
	ystarting := 10 ; Sedikit lebih ke bawah
	xstarting := 15 ; Sedikit lebih ke kanan
	xdifference := 110 ; Lebar kolom sedikit ditambah
	ydifference := 25 ; Jarak antar baris (spacing) ditambah agar tidak sesak
	xpos := xstarting
	ypos := ystarting
	ytreshhold := 320 + ystarting ; Threshold disesuaikan
		for i, v in Appraiseable
		{
			try {
				if (instr(v, "(Event)")) {
				vt := trim(strreplace(v, "(Event)", ""))
				thegui.SetFont("s9 c89b4fa", "Segoe UI") ; Warna biru muda untuk Event
				maps["cb" . vt] := thegui.Add("CheckBox", "v" vt " x" xpos " y" ypos " h20", vt)
				}else{
				thegui.SetFont("s10 ccdd6f4", "Segoe UI") ; Warna putih tulang (off-white)
				maps["cb" . v] := thegui.Add("CheckBox", "v" v " x" xpos " y" ypos " h20", v)
				}
				ypos += ydifference
				if (ypos >= ytreshhold) {
					xpos += xdifference
...
global
	...
	; --- Mengambil data dari Wiki (Fischipedia) ---
	try {
		...
		; Kalau gagal ambil dari wiki, biarkan kosong (tidak ada backup GitHub lagi)
		msgbox "Gagal mengambil data dari Wiki.", "Error", "0x10"
	}

	; --- Bagian GitHub yang dulu ada di sini sudah DIHAPUS ---
...
global
	...
	thegui.BackColor := "1E1E2E" ; Modern Dark Blue/Black Background
    
	MakeGUI()
	
    ; --- Bagian Kanan (Settings) ---
	thegui.SetFont("s10 ccdd6f4", "Segoe UI")
	AppraiseAnywhereOption := thegui.Add("Checkbox", "x" otherstarting+20 " y120 +Background1E1E2E w160", "Appraise Anywhere")
	AppraiseAnywhereOption.value := AAO
	AppraiseAnywhereOption.OnEvent("Click", UpdateFile1Thing)
	
    ; --- Header Title ---
	thegui.SetFont("s14 c89b4fa w700", "Segoe UI") ; 
	thegui.Add("Text", "x" otherstarting+10 " y10 w220 ", "Auto Appraise")
	
    ; --- Subtitle ---
	thegui.SetFont("s9 c6c7086 w400 q2", "Segoe UI") ; 
	thegui.Add("Text", "x" otherstarting+10 " y40 w220 ", "Made by : HaveMine")
	
    ; --- Info Text ---
	thegui.SetFont("S9 Ca6e3a1 W600", "Segoe UI") ; 
	thegui.Add("Text", "x" otherstarting " y70 w240 ", "[F6] Start`n[F7] Box`n[Alt] Stop")
	
    ; --- GroupBox (Border) ---
	thegui.SetFont("s10 c585b70", "Segoe UI") ; Border warna gelap
	heightofgroupbox := (reachedmax) ? (ytreshhold-ystarting)+20 : ypos+15
	thegui.Add("groupbox", "x" xstarting-8 " y" ystarting-10 " w" otherstarting-xstarting-5 " h" heightofgroupbox)
	
    ; --- Buttons ---
    ; Tombol tidak bisa diwarnai backgroundnya secara native di AHK v2 tanpa library tambahan,
    ; tapi kita set fontnya agar rapi.
	thegui.SetFont("s9 cBlack", "Segoe UI") 
	
	TakeAppraisalSettings := thegui.add("button", "x" otherstarting+20 " y150 w125 h28", "Change Position")
	TakeAppraisalSettings.OnEvent("Click", (*) => TakeAppraiseAnywhereSettings())
	
	ToggleRedBoxThing := thegui.add("button", "x" otherstarting+20 " y185 w125 h28", "Toggle Highlight")
	ToggleRedBoxThing.OnEvent("Click", (*) => ToggleHighlight())
	
	AutoUpdateMutation := thegui.add("button", "x" otherstarting+20 " y220 w125 h28", "Update Mutation")
	AutoUpdateMutation.OnEvent("Click", (*) => UpdateMutation())
	
	uncheckallboxes := thegui.add("button", "x" otherstarting+20 " y255 w60 h25", "Uncheck")
	uncheckallboxes.OnEvent("Click", (*) => dosomethingtoboxes(0))
	
	checkallboxes := thegui.add("button", "x" otherstarting+85 " y255 w60 h25", "Check All")
	checkallboxes.OnEvent("Click", (*) => dosomethingtoboxes(1))
	
    ; Tombol panah kecil
	extrasettings := thegui.add("button", "x" otherstarting+150 " y150 w25 h28", ">")
	extrasettings.OnEvent("Click", (*) => ShowAnotherUI())
	
	OpenTheMutationUp := thegui.add("button", "x" otherstarting+150 " y220 w25 h28", ">")
	OpenTheMutationUp.OnEvent("Click", (*) => run(file2))
	
    ; Retake Box Button (Paling Bawah)
	thegui.SetFont("s10 w600", "Segoe UI")
	retakeboxsettings := thegui.add("button", "x" otherstarting+20 " y295 w155 h35", "Retake Box Settings")
	retakeboxsettings.OnEvent("Click", takeocrbox)

	thegui.Title := "Auto Appraisal"
...
global
	WinGetPos( &Xx2, &Yy2, &Width, &Height, thegui)
	; Menyesuaikan posisi settings agar nempel rapi di sebelah kanan GUI utama
	thegui2.show("x" Xx2+Width+5 " y" Yy2 " w160 h290") 
